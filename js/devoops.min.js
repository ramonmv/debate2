...
function LoadCalendarScript(callback) {
    function LoadFullCalendarScript() {
        if (!$.fn.fullCalendar)
            $.getScript('plugins/fullcalendar/fullcalendar.js',
                    callback);
        else if (callback && typeof callback === "function")
            callback();
    }
    if (!$.fn.moment)
        $.getScript('plugins/moment/moment.min.js',
...
function LoadTimePickerScript(callback) {
    if (!$.fn.timepicker)
        $.getScript('plugins/jquery-ui-timepicker-addon/jquery-ui-timepicker-addon.min.js',
                callback);
    else if (callback && typeof callback === "function")
        callback();
}
...
function LoadDataTablesScripts(callback) {
    function LoadDatatables() {
        $.getScript('plugins/datatables/jquery.dataTables.js',
                function () {
                    $.getScript('plugins/datatables/ZeroClipboard.js',
                            function () {
                                $.getScript('plugins/datatables/TableTools.js',
                                        function () {
...
function LoadXChartScript(callback) {
    function LoadXChart() {
        $.getScript('plugins/xcharts/xcharts.min.js',
                callback);
    }
    function LoadD3Script() {
        if (!$.fn.d3)
            $.getScript('plugins/d3/d3.v3.min.js',
                    LoadXChart);
        else
            LoadXChart();
    }
...
function LoadFlotScripts(callback) {
    ...
    function LoadFlotTimeScript() {
        $.getScript('plugins/flot/jquery.flot.time.js',
                callback);
    }
    if (!$.fn.flot)
        LoadFlotScript();
...
function LoadMorrisScripts(callback) {
    function LoadMorrisScript() {
        if (!$.fn.Morris)
            $.getScript('plugins/morris/morris.min.js',
                    callback);
        else if (callback && typeof callback === "function")
            callback();
    }
...
function LoadFancyboxScript(callback) {
    if (!$.fn.fancybox)
        $.getScript('plugins/fancybox/jquery.fancybox.js',
                callback);
    else if (callback && typeof callback === "function")
        callback();
}
function LoadKnobScripts(callback) {
    if (!$.fn.knob)
        $.getScript('plugins/jQuery-Knob/jquery.knob.js',
                callback);
    else if (callback && typeof callback === "function")
        callback();
}
function LoadSparkLineScript(callback) {
    if (!$.fn.sparkline)
        $.getScript('plugins/sparkline/jquery.sparkline.min.js',
                callback);
    else if (callback && typeof callback === "function")
        callback();
}
        ...
        for (var i = 0; i < particles.length; i++) {
            ...
            for (var n = 0; n < particles.length; n++) {
                ...
                if (distance < 200) {
                    ctx.beginPath();
                    ctx.lineWidth = 1;
                    ctx.moveTo(p.location.x,
                            p.location.y);
                    ctx.lineTo(p2.location.x,
                            p2.location.y);
...
function drawGoogleChart(chart_data,
        chart_options,
        element,
        chart_type) {
...
function drawMap(lon,
    ...
    var lonlat = new OpenLayers.LonLat(lon,
            lat);
    map.setCenter(lonlat.transform(epsg4326,
            projectTo),
            zoom);
    var layerGuest = new OpenLayers.Layer.Vector("You are here");
    var guestMarker = new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Point(lon,
            lat).transform(epsg4326,
            projectTo));
...
function DashboardTabChecker() {
    $('#content').on('click',
            ...
            function (e) {
                ...
                $('#' + 'dashboard-' + attr).css('visibility',
                        'visible').css('position',
                        'relative');
...
function SparkLineDrawBarGraph(elem,
        arr,
        color) {
    if (color)
...
function Table2Json(table) {
    ...
    OpenModalBox('Table to JSON values',
            result_json);
}
function FlotGraph1() {
    var data = [],
...
function FlotGraph2() {
    ...
    $("#box-two-content").bind("plothover",
            function (event,
                    pos,
                    item) {
                latestPosition = pos;
                if (!updateLegendTimeout)
                    updateLegendTimeout = setTimeout(updateLegend,
                            50);
            });
}
function FlotGraph3() {
...
function xGraph1() {
    ...
    var myChart = new xChart('line-dotted',
            data,
            '#xchart-1',
            opts);
}
    ...
    $.getJSON(jsonURL,
            function (result) {
                ...
                $('#coindesk-xchart').resize(function () {
                    ...
                    graphXChartResize = setTimeout(DrawCoinDeskXCharts,
                            500);
                });
                DrawCoinDeskXCharts();
                $.each(result.bpi,
                ...
                $('#coindesk-google-chart').resize(function () {
                    ...
                    graphGChartResize = setTimeout(DrawCoinDeskGoogleCharts,
                            500);
                });
...
function TestTable2() {
    ...
    header_inputs.on('keyup',
            function () {
                oTable.fnFilter(this.value,
                        header_inputs.index(this));
            }).on('focus',
            function () {
...
function SparklineLoop() {
    SparkLineDrawBarGraph($('#sparkline-1'),
            sparkline_arr_1.map(SmallChangeVal));
    SparkLineDrawBarGraph($('#sparkline-2'),
            sparkline_arr_2.map(SmallChangeVal),
            '#7BC5D3');
    SparkLineDrawBarGraph($('#sparkline-3'),
            sparkline_arr_3.map(SmallChangeVal),
            '#B25050');
}
...
function LoadTestMap() {
    $.getJSON("http://www.telize.com/geoip?callback=?",
            function (json) {
                ...
                var map1 = drawMap(json.longitude,
                        json.latitude,
                        "map-1",
                        map1_layers);
                $("#map-1").resize(function () {
                    setTimeout(map1.updateSize(),
                            500);
                });
                ...
                var map2 = drawMap(json.longitude,
                        json.latitude,
                        "map-2",
                        map2_layers);
                $("#map-2").resize(function () {
                    setTimeout(map2.updateSize(),
                            500);
                });
                ...
                var map3 = drawMap(json.longitude,
                        json.latitude,
                        "map-3",
                        map3_layers);
                $("#map-3").resize(function () {
                    setTimeout(map3.updateSize(),
                            500);
                });
            });
}
function FullScreenMap() {
    $.getJSON("http://www.telize.com/geoip?callback=?",
            function (json) {
                ...
                var map_fs = drawMap(json.longitude,
                        json.latitude,
                        "full-map",
                        map1_layers);
            });
    ...
    $('#defaultForm').bootstrapValidator({
        ...
        fields: {
            ...
            email: {
                validators: {
                    notEmpty: {
                        message: 'É necessário o preenchimento do email.'
                    },
                    emailAddress: {
...
function CreateAllSliders() {
    ...
    slider_range.slider({
        ...
        slide: function (event,
                ui) {
            slider_range_amount.val("$" + ui.values[
                    0
            ] + " - $" + ui.values[
                    1
            ]);
    ...
    slider_range_amount.val("$" + slider_range.slider("values",
            0) + " - $" + slider_range.slider("values",
            1));
...
function DrawCalendar() {
    ...
    var calendar = $('#calendar').fullCalendar({
        ...
        select: function (start,
                ...
                allDay) {
            var form = $('<form id="event_form">' + '<div class="form-group has-success has-feedback">' + '<label">Event name</label>' + '<div>' + '<input type="text" id="newevent_name" class="form-control" placeholder="Name of event">' + '</div>' + '<label>Description</label>' + '<div>' + '<textarea rows="3" id="newevent_desc" class="form-control" placeholder="Description"></textarea>' + '</div>' + '</div>' + '</form>');
            var buttons = $('<button id="event_cancel" type="cancel" class="btn btn-default btn-label-left">' + '<span><i class="fa fa-clock-o txt-danger"></i></span>' + 'Cancel' + '</button>' + '<button type="submit" id="event_submit" class="btn btn-primary btn-label-left pull-right">' + '<span><i class="fa fa-clock-o"></i></span>' + 'Add' + '</button>');
            OpenModalBox('Add event',
                    form,
                    buttons);
            $('#event_cancel').on('click',
                    function () {
                        CloseModalBox();
                    });
    ...
    $('#new-event-add').on('click',
            function (event) {
                ...
                if (event_name != '') {
                    ...
                    event_template.data('eventObject',
                            eventObject);
...
$(document).ready(function () {
    ...
    $('.main-menu').on('click',
            ...
            function (e) {
                ...
                } else if (li.find('a.dropdown-toggle').hasClass('active-parent')) {
    ...
    $('#screen_unlock').on('mouseover',
            function () {
                ...
                var form = $('<div class="form-group"><label class="control-label">Username</label><input type="text" class="form-control" name="username" /></div>' + '<div class="form-group"><label class="control-label">Password</label><input type="password" class="form-control" name="password" /></div>');
